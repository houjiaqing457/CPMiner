...
class Pipeline:
    ...
    def process_incomplete_samples(self):
        ...
        for sample_dir in sample_dirs:
            ...
            # 3) 建库（scaffold）
            blast_db = self.output_root / "blast_dbs" / f"{sample_id}_db"
            subprocess.run([
                "makeblastdb", "-in", str(scaffold_fasta),
                "-dbtype", "nucl", "-out", str(blast_db)
            ], check=True)
    ...
    def merge_fasta(self):
        ...
        for gene in self.genes:
            ...
            # ----------- 3. 写出合并文件 -----------
            merged_file = self.output_root / "merged_fasta" / f"all_samples_{gene}.fasta"
    ...
    def run_alignments(self):
        ...
        for fasta_file in merged_dir.glob("*.fasta"):
            ...
            subprocess.run(f"mafft --auto --thread {self.args.threads} \"{fasta_file}\" > \"{output_file}\"", shell=True, check=True)
    ...
    def run_phylogenetic_tree(self, mode="coalescent"):
        ...
        run_tree_script = os.path.join(os.path.dirname(os.path.realpath(__file__)), "run_tree.py")
        cmd = [
            sys.executable,
            run_tree_script,
            "-m", mode,
