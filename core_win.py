...
def pick_script(basename: str) -> str:
    """basename 不带 .py；Windows 用 *_win.py，其它平台用原始 .py"""
    return f"{basename}_win.py" if IS_WIN else f"{basename}.py"
...
def run_cat_mafft(result_dir):
    ...
    cmd = [
        sys.executable,
        pick_script("cat_mafft"),   # ← 这里改成自动挑选 *_win.py
...
def run_trimal(input_dir, output_dir, mode="automated1"):
    ...
    cmd = [
        sys.executable,
        pick_script("run_trimal"),  # ← 自动挑选 *_win.py
        "-i", input_dir,
        "-o", output_dir,
        "-m", mode
    ]
...
def run_tree_building(args, workdir):
    ...
    tree_script = os.path.join(os.path.dirname(__file__), pick_script("run_tree"))
