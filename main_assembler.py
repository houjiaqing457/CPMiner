...
def Process_Contigs(contigs, max_weight, slice_len, reads_dict, soft_boundary=(0,0)):
    """
    ...
    left, right = soft_boundary
    for i, contig in enumerate(contigs):
        if len(contig[0]) > 2:
            cut_value = Quartile(contig[0])[0]
            cut_pos   = find_position(contig[0], cut_value)
            # 只用 right 来裁剪右端
            if cut_pos != -1 and cut_pos + right + 1 < len(contig[0]):
                while len(contig[0]) > cut_pos + right + 1:
                    contig[0].pop()
                    contig[1].pop()
...
def Get_Contig_v6(_reads_dict, slice_len, _dict, seed, kmer_size, iteration = 1024, soft_boundary = (0,0)):
    """
    获取最优的contig
...
if __name__ == '__main__':
    ...
    pars.add_argument('-sb', '--soft_boundary', metavar=('LEFT', 'RIGHT'), type=int, nargs=2, default=[0, 0],
                  help='Soft boundary left and right (e.g., -sb 200 200)')
